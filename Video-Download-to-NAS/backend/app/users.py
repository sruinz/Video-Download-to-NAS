...
from ..models import UserRegister, UserInfo, UserUpdate, UserRoleUpdate, UserQuotaUpdate, UserRateLimitUpdate, FolderOrganizationUpdate, FolderOrganizationResponse
...
@router.get("/me/folder-organization", response_model=FolderOrganizationResponse)
async def get_folder_organization(
    current_user: User = Depends(get_current_user)
):
    """
    현재 사용자의 폴더 구성 모드를 조회합니다.
    
    Returns:
        FolderOrganizationResponse: 현재 설정된 폴더 구성 모드
    """
    return FolderOrganizationResponse(mode=current_user.folder_organization_mode)


@router.patch("/me/folder-organization", response_model=FolderOrganizationResponse)
async def update_folder_organization(
    folder_update: FolderOrganizationUpdate,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """
    현재 사용자의 폴더 구성 모드를 업데이트합니다.
    ...
    Args:
        folder_update: 폴더 구성 모드 업데이트 요청
        current_user: 현재 인증된 사용자
        db: 데이터베이스 세션
    
    Returns:
        FolderOrganizationResponse: 업데이트된 폴더 구성 모드
